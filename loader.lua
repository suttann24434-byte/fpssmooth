# Simpan kode Lua final ke file agar bisa diunduh
lua_code = """
-- SuttannHub vSafe
-- Safe version untuk Roblox executor
-- FPS Boost dan Key System aman
...
local HWID = (pcall(function() return gethwid() end) and gethwid()) or tostring(game:GetService("RbxAnalyticsService"):GetClientId())
...
-- Fungsi safe untuk file HWID
local function LoadValidHWIDs()
    local hwids = {}
    if pcall(function() return isfile and isfile(keyFile) end) then
...
local function SaveHWID(hwid)
    ...
    if not hwids[hwid] and pcall(function() return writefile end) then
        local data = ""
...
-- Jika HWID sudah tersimpan, langsung load script utama
if HasAccess() then
    pcall(function()
...
-- FPS Boost safe
local FPSButton = Instance.new("TextButton", LocalPlayer:WaitForChild("PlayerGui"))
...
file_path = "/mnt/data/SuttannHub_vSafe.lua"
