...
local function LoadValidHWIDs()
    local hwids = {}
    if pcall(function() return isfile(keyFile) end) and isfile(keyFile) then
...
local function SaveHWID(hwid)
    ...
    if not hwids[hwid] and pcall(function() return writefile end) and writefile then
        local data = ""
...
-- Function to load main script safely
...
-- If HWID is already saved, load main script
if HasAccess() then
    LoadMainScript()
    return
end
